...
describe('Transkribus MCP Server — smoke tests', () => {
  it('registers 297 tools for the full server', () => {
    ...
    expect(toolCount(server)).toBe(297);
  });
  ...
  it('registers 124 collection tools', () => {
    const server = freshServer();
    registerCollectionCoreTools(server);
    registerCollectionDocumentTools(server);
    registerCollectionPageTools(server);
    ...
    registerCollectionActivityTools(server);
    registerCollectionTagTools(server);
    expect(toolCount(server)).toBe(124);
  });
  ...
  // Entry point tests
  it('registers 129 tools for collections entry point', () => {
    const server = freshServer();
    registerAuthTools(server);
    registerCollectionCoreTools(server);
    registerCollectionDocumentTools(server);
    ...
    registerCollectionActivityTools(server);
    registerCollectionTagTools(server);
    expect(toolCount(server)).toBe(129); // 5 auth + 124 collections
  });
